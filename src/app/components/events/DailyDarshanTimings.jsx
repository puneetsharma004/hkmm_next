      ...
      <div className="relative max-w-6xl mx-auto z-10">
        ...
        {nextAarti && (
          <motion.div
            ...
            transition={{ duration: 0.8 }}
            viewport={{ once: true }}
            className="bg-gradient-to-r from-orange-100/80 to-amber-100/80 rounded-2xl p-6 mb-12 border border-primary/40 border-opacity-60 backdrop-blur-xl shadow-lg"
          >
            <div className="text-center">
              <h3 className="text-3xl font-bold text-gray-800 mb-2">Next Aarti</h3>
              <div className="text-2xl font-bold text-primary mb-2">{nextAarti.name}</div>
              <div className="text-xl text-primary mb-2 flex items-center justify-center gap-2">
                <FaRegClock />
                {nextAarti.time}
              </div>
              <div className="text-lg text-gray-700">
                Starts in: <span className="font-bold text-primary">{getTimeUntilNext()}</span>
              </div>
            </div>
          </motion.div>
        )}
          ...
          {aartiSchedule.map((aarti, index) => {
            ...
            return (
              <motion.div
                ...
                className={`rounded-2xl shadow-2xl p-6 border backdrop-blur-xl transition-all duration-300 group ${isNext
              ...
              >
                ...
                <div className="flex items-center justify-between mb-4">
                  <div className="text-4xl text-primary">{aarti.icon}</div>
                  <div className="text-right">
                    <div className={`text-2xl font-bold ${isNext ? 'text-primary' : 'text-primary'}`}>
                      {aarti.time}
                    </div>
                  </div>
                </div>
                ...
                {/* Aarti Name */}
                <h3 className={`text-xl font-bold mb-3 transition-colors duration-300 ${isNext ? 'text-gray-800' : 'text-gray-800 group-hover:text-primary'
                  }`}>
                  {aarti.name}
                </h3>
        ...
        <motion.div
          ...
          viewport={{ once: true }}
          className="mt-12 bg-gradient-to-r from-blue-100/80 to-indigo-100/80 rounded-2xl p-8 border border-blue-300 border-opacity-60 backdrop-blur-sm shadow-lg"
        >
          <h3 className="text-2xl font-bold text-center text-gray-800 mb-6">Temple Timings</h3>
          <div className="grid md:grid-cols-2 gap-8">
            <div className="text-center">
              <div className="text-4xl mb-3 text-primary flex justify-center items-center">
                <FaSun />
              </div>
              <h4 className="font-bold text-primary mb-2">Morning Session</h4>
              <p className="text-gray-700">4:30 AM - 12:30 PM</p>
              <p className="text-sm text-gray-600 mt-1">Daily darshan and morning activities</p>
            </div>
            <div className="text-center">
              <div className="text-4xl mb-3 text-primary flex justify-center items-center">
                <BsFillSunsetFill />
              </div>
              <h4 className="font-bold text-primary mb-2">Evening Session</h4>
              <p className="text-gray-700">4:00 PM - 8:30 PM</p>
              <p className="text-sm text-gray-600 mt-1">Evening darshan and night aarti</p>
            </div>
          </div>

          <div className="text-center mt-6 pt-6 border-t border-gray-400">
            <p className="text-gray-700 text-sm flex items-center justify-center gap-2">
              <FaDoorClosed className="text-primary" />
              Temple remains <span className="text-primary font-semibold">closed</span> from 12:30 PM to 4:00 PM for deity rest
            </p>
          </div>
        </motion.div>
