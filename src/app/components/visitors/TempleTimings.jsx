    ...
    <section className="relative py-16 px-4 overflow-hidden bg-gradient-to-br from-orange-50 via-amber-50 to-yellow-50">
      ...
      <div className="relative max-w-6xl mx-auto z-10">
        ...
        <motion.div
          ...
          transition={{ duration: 0.8 }}
          viewport={{ once: true }}
          className={`mb-12 rounded-2xl p-6 border backdrop-blur-xl shadow-lg border-opacity-60 ${isOpen
        ...
        >
          <div className="text-center">
            ...
            <h3 className="text-2xl font-bold text-gray-800mb-2">
              Temple is currently {isOpen ? 'OPEN' : 'CLOSED'}
            </h3>
            {!isOpen && (
              <p className="text-orange-600 mt-2 flex items-center justify-center gap-2">
                <FaBell />
        ...
        <div className="grid lg:grid-cols-3 gap-8 mb-16">
          {dailySchedule.map((schedule, index) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className={`rounded-2xl p-6 border backdrop-blur-xl transition-all duration-300 group shadow-xl ${schedule.highlight
                ? 'bg-gradient-to-br from-primary/20 to-primary/20 border-primary/60 border-opacity-80 hover:border-opacity-90'
                : 'bg-white/10 border-primary border-opacity-60'
                }`}
            >
              <div className="text-center">
                <div className="text-5xl mb-4 text-primary flex justify-center">{schedule.icon}</div>
                <h3 className={`text-xl font-bold mb-2 ${schedule.highlight ? 'text-gray-800' : 'text-gray-600'}`}>
                  {schedule.session}
                </h3>
                <div className={`text-2xl font-bold mb-4 flex items-center justify-center gap-2 ${schedule.highlight ? 'text-primary' : 'text-gray-500'}`}>
                  <FaClock />
                  {schedule.time}
                </div>
                ...
                <div className="space-y-2">
                  {schedule.activities.map((activity, actIndex) => (
                    <div
                      key={actIndex}
                      className={`text-sm px-3 py-1 rounded-full ${schedule.highlight
                        ? 'bg-orange-100/20  text-gray-700'
        ...
        <motion.div
          ...
          className="mb-16"
        >
          <h3 className="text-2xl font-bold text-center text-gray-800 mb-8">Special Occasion Timings</h3>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {bestTimes.map((time, index) => (
              <motion.div
                ...
                viewport={{ once: true }}
                className="flex items-start space-x-4 p-4 bg-white/10 rounded-xl shadow-lg"
              >
                ...
                <div className="flex-1">
                  ...
                  <p className="text-gray-600 text-sm mb-2">{time.reason}</p>
                  <span className={`text-xs px-2 py-1 rounded-full flex items-center gap-1 w-fit ${time.crowd === 'Low' ? 'bg-green-100 text-green-800' :
                    time.crowd === 'Medium' ? 'bg-yellow-100 text-yellow-800' :
